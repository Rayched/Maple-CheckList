...
import EditIncomeForms from "../_components/EditIncomeForms";
...
export default async function EditCharIncomePage({params, searchParams}: I_EditCharIncomePageProps){
    const {charname} = await params;
    const {ocid} = await searchParams;

    let ocidvalue: string|undefined = ocid;

    if(ocidvalue === "" || !ocidvalue){
        const Ocids = await GetOcids(charname);
        ocidvalue = Ocids?.ocid;
    }
